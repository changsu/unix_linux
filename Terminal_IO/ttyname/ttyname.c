...
// keep a list of directory names
struct devdir {
...
// add dirname to current directory list
...
cleanup(void) {
  ...
  while (ddp != NULL) {
    nddp = ddp->d_next;
    free(ddp->d_name); // need to free, strdup is "hidden malloc"
    free(ddp);
    ddp = nddp;
  }
  head = NULL;
...
static char* searchdir(char *dirname, struct stat* fdstatp) {
  ...
  while ((dirp = readdir(dp)) != NULL) {
    ...
    // get the fd stat of current device
    if (stat(pathname, &devstat) < 0) {
      continue;
    }

    // if directory, add to search list
...
char *ttname(int fd) {
  ...
  // return null is not a terminal device
  if (isatty(fd) == 0) {
    return (NULL);
  }
